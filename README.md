...
```The fitness() function calculates the total value of the items selected in the chromosome. It first computes the total weight and total value of the packed items. If the total weight exceeds the maximum allowed weight (15 kg), the function returns 0 to mark the solution as invalid. This penalty prevents the algorithm from selecting overweight knapsack combinations.
